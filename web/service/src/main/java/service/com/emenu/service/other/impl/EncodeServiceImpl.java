...
public class EncodeServiceImpl implements EncodeService {
    ...
    public String encodeSql(String input){
        ...
        input = ESAPI.encoder().encodeForSQL(MYSQLCODEC, input);
        return input;
    }
    ...
    public String encodeHtmlTag(String input) {
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            String hex = Codec.getHexForNonAlphanumeric(c);  //转换为16进制
            if (hex == null) {
                sb.append("" + c);
                continue;
            }
            ...
            // 检查 非法字符
            if ((c <= 0x1f && c != '\t' && c != '\n' && c != '\r') || (c >= 0x7f && c <= 0x9f)) {
                hex = REPLACEMENT_HEX; // Let's entity encode this instead of
                // returning it
                c = REPLACEMENT_CHAR;
            }
    ...
    private static synchronized Map<Character, String> mkCharacterToEntityMap() {
        ...
        map.put((char) 60, "lt"); /* 小于符号 */
        map.put((char) 62, "gt"); /* 大于符号 */
        ...
        map.put((char) 223, "szlig"); /*
									 * Latin small letter sharp sXCOMMAX German
									 * Eszett
									 */
    ...
    public String encodeAll(String input){
        ...
        input = this.encodeHtmlTag(input);
        // input = this.encodeSql(input);
        return input;
    }
}
