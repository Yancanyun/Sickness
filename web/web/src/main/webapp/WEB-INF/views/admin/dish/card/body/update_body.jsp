			...
			<div class="panel-body">
				<form class="form-horizontal J_operForm">
					...
					<div class="form-group">
						...
						<div class="col-sm-3">
							<input type="text" class="col-sm-11" value="${costCardDto.name}">
						</div>
						<a class="btn btn-success margin-bottom-15 J_add" href="javascript:;"><i class="fa fa-plus"></i>&nbsp;添加原材料</a>
					</div>
					<input type="hidden" name="id" Class="J_costCardId" value="${costCardDto.id}"/>
					<input type="hidden" class="J_dishId" name="dishId" value="${costCardDto.dishId}"/>
					<h4>原配料管理</h4>
					<hr/>
					<div class="table-responsive">
						<table class="table table-hover table-bordered">
							...
							<tbody class="J_tbody">
							<c:forEach items="${costCardDto.costCardItemDtos}" var="costCardItemDto">
								<tr data-ingredient-id="${costCardItemDto.ingredientId}">
									<th>${costCardItemDto.ingredientName}</th>
									<c:if test="${costCardItemDto.itemType==0}">
										<th class="J_itemType" data-itemType="${costCardItemDto.itemType}">主料</th>
									</c:if>
									<c:if test="${costCardItemDto.itemType==1}">
										<th class="J_itemType" data-itemType="${costCardItemDto.itemType}">辅料</th>
									</c:if>
									<c:if test="${costCardItemDto.itemType==2}">
										<th class="J_itemType" data-itemType="${costCardItemDto.itemType}">调料</th>
									</c:if>
									<th>${costCardItemDto.costCardUnit}</th>
									<th class="J_otherCount">${costCardItemDto.otherCount}</th>
									<th>${costCardItemDto.netCount}</th>
									<c:if test="${costCardItemDto.isAutoOut==0}">
										<th>否</th>
									</c:if>
									<c:if test="${costCardItemDto.isAutoOut==1}">
										<th>是</th>
									</c:if>
									<th>
									...
									<input type="hidden" class="J_ingredientInfo" name="ingredientInfo" value='${costCardItemDto.ingredientInfo}'/>
								</tr>
							</c:forEach>
							</tbody>
						</table>
					...
					<div class="form-group">
						...
						<div class="col-sm-6">
							<input class="w200 J_price J_mainCost" type="text" data-valid-rule="isNull | notNegativeNumber " data-valid-tip="请输入大于0的数字|请输入大于0的数字" name="mainCost" value="${costCardDto.mainCost}">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label">辅料成本</label>
						<div class="col-sm-6">
							<input class="w200 J_price J_assistCost" type="text"  name="assistCost" value="${costCardDto.assistCost}" data-valid-rule="isNull | notNegativeNumber " data-valid-tip="请输入大于0的数字|请输入大于0的数字">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label">调料成本</label>
						<div class="col-sm-6">
							<input class="w200 J_price J_deliciousCost" type="text" data-valid-rule="isNull | notNegativeNumber " data-valid-tip="请输入大于0的数字|请输入大于0的数字"  name="deliciousCost" value="${costCardDto.deliciousCost}">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label">标准成本</label>
						<div class="col-sm-6">
					        <input class="w200 J_standardCost" readonly type="text" value="${costCardDto.standardCost}">
						</div>
					</div>
				</form>
			</div>
